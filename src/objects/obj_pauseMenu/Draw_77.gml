...
//Disable alpha blending (we just want RGB from screen) gpu_set_blendenable(false);
if (pause) //draw frozen image to screen while paused
{	
	surface_set_target(application_surface);
	
	//draw_text(885, 110, string("PP: "));
...
if (keyboard_check_pressed(vk_escape)) && (!instance_exists(obj_RPGtransition)) //Toggle pause (Whatever condition/trigger you like)
{	
	if (!pause) //pause now
	{
		pause = true;
		...
		audio_pause_all();
		//NOTE:
		//If you need to pause anything like animating sprites, tiles, room backgrounds etc // you need to do that separately, unfortunately!
